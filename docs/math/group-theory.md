...
在数学中，**群**（group）是由一个集合和一个二元运算所组成的，符合「群公理」的代数结构。
...
1. **封闭性**：对于 $G$ 中所有的 $a, b$，$a \cdot b$ 也在 $G$ 中。
...
4. **逆元**（inverse element）：对于 $G$ 中所有的 $a$，总存在 $G$ 中的一个元素 $b$ 使 $a \cdot b = b \cdot a = e$，此处 $e$ 为单位元，称 $b$ 为 $a$ 的逆元，记为 $a^{-1}$。
...
（注意这里不一定是我们一般所熟知的四则运算加法和乘法）所组成的，  
且满足如下性质的代数结构 $(R,+,\cdot)$：
...
**子群** 是包含在更大的群 $G$ 内的一个群 $H$：它具有由 $G$ 的元素构成的子集和相同的运算。这意味着 $G$ 的单位元必然包含在 $H$ 中，并且如果 $h_{1}$ 和 $h_{2}$ 都在 $H$ 中，那么 $h_{1} \cdot h_{2}$ 和 $h_{1}^{-1}$ 也在 $H$ 中。这样 $H$ 中的元素，和在 $G$ 上的限制在 $H$ 的运算，也形成了一个群。

即，若 $(G,\cdot)$ 是群，$H$ 是 $G$ 的非空子集，且 $(H,\cdot)$ 也是群，则称 $(H,\cdot)$ 是 $(G,\cdot)$ 的 **子群**。

群 $G$ 的子集 $H$ 是子群的一个充分必要条件：对于所有的 $g, h \in H$，都有 $g^{-1}\cdot h\in H$ 成立。
...
**商组**（quotient group）或因子群（factor group）是通过使用保留一些群结构的等价关系聚合更大群的相似元素获得的群。
